      ...
      <section className="mx-auto max-w-5xl px-6 py-16">
        ...
        <FadeInStagger className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {content.software.items.map((item) => (
            <FadeInStaggerItem
              key={item.name}
              className="rounded-2xl border border-muted-line/30 bg-white p-5"
            >
          ...
          <FadeIn className="mt-8">
            <ul className="grid gap-4 sm:grid-cols-3">
              {content.idealFor.items.map((item) => (
                <li
                  key={item}
                  className="rounded-2xl border border-indigo/15 bg-white p-5 text-base leading-relaxed text-muted"
                >
                  {item}
                </li>
              ))}

...
/**
 * The products listing, shared by /products and its translations.
 *
 * The three labels that used to be written into the markup — the "Coming Soon"
 * badge and the paid-product note — now come from content, because a page
 * published in seventeen languages cannot have English baked into its JSX.
 */
export function ProductsSections({
   ...
   * schema on the page, so the page and the structured data telling Google what
   * is on it never disagree.
   */
  const visibleProducts = content.products.filter((product) => {
    ...
    // out of: announcing "Setu Pharmacy is coming" while the free Pharmacy POS
    // is still unreleased would advertise a product nobody can see the shape of.
  ...
  return (
    <>
                ...
                {"paidSoon" in product && product.paidSoon && (
                  <div className="mt-6 rounded-xl border border-muted-line/30 bg-cream/60 p-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-muted-warm">
                      {content.labels.paidSoonEyebrow}
                    </p>
                    <p className="mt-1 text-sm text-ink/80">
                      <span className="font-semibold text-ink">{product.paidSoon.name}</span> —{" "}
                      {product.paidSoon.note} {content.labels.paidSoonSuffix}
                    </p>
                  </div>
                )}
      ...
      <LanguageLinks page="products" current={lang} />
    </>
  );
}
